...
public class 지라이슈상태_전략_호출 {
    ...
    private 지라이슈상태_전략_등록_및_실행 지라_이슈_상태_전략_확인(서버정보_데이터 서버정보) throws Exception {

        if (서버정보 == null || 서버정보.getType().isEmpty()) {
            로그.error("지라이슈 상태 전략 등록 Error: 서버정보_유형 " + 에러코드.서버_유형_정보없음.getErrorMsg());
            throw new IllegalArgumentException("지라이슈 상태 전략 등록 Error: 서버정보_유형 " + 에러코드.서버_유형_정보없음.getErrorMsg());
        }
    ...
    public List<지라이슈상태_데이터> 이슈_상태_목록_가져오기(Long 연결_아이디) throws Exception {
        ...
        지라이슈상태_전략_등록_및_실행 = 지라_이슈_상태_전략_확인(서버정보);
    ...
    public List<지라이슈상태_데이터> 프로젝트별_이슈_상태_목록_가져오기(Long 연결_아이디, String 프로젝트_아이디) throws Exception {
        ...
        if (프로젝트_아이디 == null || 프로젝트_아이디.isEmpty()) {
            로그.error("프로젝트별_이슈_상태_목록_가져오기 Error 프로젝트_아이디 " + 에러코드.파라미터_NULL_오류.getErrorMsg());
            throw new IllegalArgumentException("프로젝트별_이슈_상태_목록_가져오기 Error 프로젝트_아이디 " + 에러코드.파라미터_NULL_오류.getErrorMsg());
        }
