...
import com.arms.jira.jiraissueresolution.strategy.온프레미스_지라_이슈_해결책_전략;
...
public class 지라_이슈_해결책_전략_호출 {
    지라_이슈_해결책_전략_등록_및_실행 지라_이슈_해결책_전략_등록_및_실행;

    클라우드_지라_이슈_해결책_전략 클라우드_지라_이슈_해결책_전략;

    온프레미스_지라_이슈_해결책_전략 온프레미스_지라_이슈_해결책_전략;

    JiraInfo jiraInfo;

    @Autowired
    public 지라_이슈_해결책_전략_호출(지라_이슈_해결책_전략_등록_및_실행 지라_이슈_해결책_전략_등록_및_실행,
                           클라우드_지라_이슈_해결책_전략 클라우드_지라_이슈_해결책_전략,
                           온프레미스_지라_이슈_해결책_전략 온프레미스_지라_이슈_해결책_전략,
                           JiraInfo jiraInfo) {
    ...
    private 지라_이슈_해결책_전략_등록_및_실행 지라_이슈_해결책_전략_확인(JiraInfoDTO 연결정보) {
        ...
        if (지라_유형 == 지라_유형_정보.클라우드) {
            ...
            지라_이슈_해결책_전략_등록_및_실행.지라_이슈_해결책_전략_등록(온프레미스_지라_이슈_해결책_전략);
        }
    ...
    public List<지라_이슈_해결책_데이터_전송_객체> 이슈_해결책_목록_가져오기(Long 연결_아이디) throws Exception {
        ...
        List<지라_이슈_해결책_데이터_전송_객체> 반환할_지라_이슈_해결책_데이터전송객체_목록
                = 지라_이슈_해결책_전략_등록_및_실행.이슈_해결책_목록_가져오기(연결_아이디);
