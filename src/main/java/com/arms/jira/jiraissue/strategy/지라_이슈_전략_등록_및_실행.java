...
public class 지라_이슈_전략_등록_및_실행 {
    ...
    public Map<String,Object> 이슈_삭제_라벨_처리하기(Long 연결_아이디, String 이슈_키_또는_아이디) throws Exception {
        return this.지라_이슈_전략.이슈_삭제_라벨_처리하기(연결_아이디, 이슈_키_또는_아이디);
    }
    ...
    public List<지라_이슈_데이터_전송_객체> 이슈링크_가져오기(Long 연결_아이디, String 이슈_키_또는_아이디) {
        return this.지라_이슈_전략.이슈링크_가져오기(연결_아이디, 이슈_키_또는_아이디);
    }

    public List<지라_이슈_데이터_전송_객체> 서브테스크_가져오기(Long 연결_아이디, String 이슈_키_또는_아이디) {
        return this.지라_이슈_전략.서브테스크_가져오기(연결_아이디, 이슈_키_또는_아이디);
    }
}
