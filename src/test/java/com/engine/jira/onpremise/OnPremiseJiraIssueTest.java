...
//    }
    ...
    public List<지라이슈_데이터> 이슈링크_가져오기(String 이슈_키_또는_아이디) throws Exception {
        ...
        // 이슈 건수가 1000이 넘을때 이슈 조회를 위한 처리
        List<지라이슈_데이터> allIssues = new ArrayList<>();
        SearchResult searchResult;
    ...
    List<지라이슈_데이터> 서브테스크_가져오기(String 이슈_키_또는_아이디) throws ExecutionException, InterruptedException {
        ...
        // 이슈 건수가 1000이 넘을때 이슈 조회를 위한 처리
        List<지라이슈_데이터> 서브테스크_목록 = new ArrayList<>();
        SearchResult searchResult;
    ...
    private 지라이슈_데이터 지라이슈_데이터로_변환(Issue 지라_이슈) {
...
//        List<지라_이슈_필드_데이터_전송_객체.연결된_이슈> 연결된_이슈_목록 = new ArrayList<>();
        ...
        // 지라이슈필드_데이터.setIssuelinks(연결된_이슈_목록);


        반환할_지라이슈_데이터.setFields(지라이슈필드_데이터);



        반환할_지라이슈_데이터.setId(지라_이슈.getId().toString());
        반환할_지라이슈_데이터.setKey(지라_이슈.getKey());
        반환할_지라이슈_데이터.setSelf(지라_이슈.getSelf().toString());

        // 필드 하위 프로젝트
        반환할_지라이슈_데이터.getFields().getProject().setSelf(지라_이슈.getProject().getSelf().toString());
        반환할_지라이슈_데이터.getFields().getProject().setId(String.valueOf(지라_이슈.getProject().getId()));
        반환할_지라이슈_데이터.getFields().getProject().setKey(지라_이슈.getProject().getKey());
        반환할_지라이슈_데이터.getFields().getProject().setName(지라_이슈.getProject().getName());
        ...
        if(지라_이슈.getIssueType()!= null){
            ...
            반환할_지라이슈_데이터.getFields().getIssuetype().setId(이슈_유형_아이디);
            반환할_지라이슈_데이터.getFields().getIssuetype().setName(이슈_유형_이름);
            반환할_지라이슈_데이터.getFields().getIssuetype().setSelf(이슈_유형_주소);
        }


        // 이슈 summary
//        반환할_지라이슈_데이터.getFields().setSummary(지라_이슈.getSummary());

        // 이슈 description
//        반환할_지라이슈_데이터.getFields().setDescription(지라_이슈.getDescription());

        // 이슈 보고자
        반환할_지라이슈_데이터.getFields().getReporter().setAccountId(지라_이슈.getReporter().getName());
        반환할_지라이슈_데이터.getFields().getReporter().setEmailAddress(지라_이슈.getReporter().getEmailAddress());

        // 이슈 담당자
        반환할_지라이슈_데이터.getFields().getAssignee().setAccountId(지라_이슈.getAssignee().getName());
        반환할_지라이슈_데이터.getFields().getAssignee().setEmailAddress(지라_이슈.getAssignee().getEmailAddress());

        // 이슈 라벨
        Set<String> 지라_라벨 = 지라_이슈.getLabels(); //HashSet 반환
        if (지라_라벨 != null) {
            List<String> 라벨_목록 = new ArrayList<>(지라_라벨);
            반환할_지라이슈_데이터.getFields().setLabels(라벨_목록);
        } else {
            반환할_지라이슈_데이터.getFields().setLabels(Collections.emptyList());
        }
...
//            }
//
//            반환할_지라이슈_데이터.getFields().getIssuelinks().add(연결된_이슈_필드);
//
//        }
...
//
//            if (반환할_지라이슈_데이터.getFields().getSubtasks() == null) {
//                반환할_지라이슈_데이터.getFields().setSubtasks(new ArrayList<>());
//            }
//            반환할_지라이슈_데이터.getFields().getSubtasks().add(서브테스크_객체);
//        }
        ...
        if(지라_이슈.getPriority() != null) {
            ...
            반환할_지라이슈_데이터.getFields().getPriority().setSelf(이슈_우선순위_주소);
            반환할_지라이슈_데이터.getFields().getPriority().setId(이슈_우선순위_이이디);
            반환할_지라이슈_데이터.getFields().getPriority().setName(이슈_우선순위_이름);
        }
        // 상태 값
        if(지라_이슈.getStatus() != null){
            ...
            반환할_지라이슈_데이터.getFields().getStatus().setId(이슈_상태_아이디);
            반환할_지라이슈_데이터.getFields().getStatus().setName(이슈_상태_이름);
            반환할_지라이슈_데이터.getFields().getStatus().setDescription(이슈_상태_설명);
            반환할_지라이슈_데이터.getFields().getStatus().setSelf(이슈_상태_주소);
        }
        // 해결책
        if(지라_이슈.getResolution()!= null){
            ...
            반환할_지라이슈_데이터.getFields().getResolution().setDescription(이슈_해결책_설명);
            반환할_지라이슈_데이터.getFields().getResolution().setId(이슈_해결책_아이디);
            반환할_지라이슈_데이터.getFields().getResolution().setName(이슈_해결책_이름);
            반환할_지라이슈_데이터.getFields().getResolution().setSelf(이슈_해결책_주소);
        }
